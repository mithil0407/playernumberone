-- Repair existing /stylist funnel installs so checkout, payment confirmation,
-- and post-payment intake all use the same schema expected by the app.

ALTER TABLE IF EXISTS public.style_scan_leads
  ADD COLUMN IF NOT EXISTS first_touch_at timestamp with time zone,
  ALTER COLUMN attribution_payload SET DEFAULT '{}'::jsonb;

UPDATE public.style_scan_leads
SET first_touch_at = created_at
WHERE first_touch_at IS NULL;

ALTER TABLE IF EXISTS public.stylist_orders
  ADD COLUMN IF NOT EXISTS first_touch_at timestamp with time zone,
