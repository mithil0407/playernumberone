...
// ── Stylist Funnel Order Confirmation Email ───────────────────────────────

export async function sendStylistOrderConfirmationEmail(
  data: AUOrderConfirmationEmailData
): Promise<{ success: boolean; error?: string }> {
  try {
    const transporter = getTransporter();
    ...
    const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Your ICONIK Style Blueprint is Confirmed</title></head>
<body style="margin:0;padding:0;background-color:#faf8f3;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background-color:#faf8f3;padding:40px 20px;"><tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
<tr><td style="background:#111111;padding:40px 40px 32px;text-align:center;">
<h1 style="margin:0;color:#ffffff;font-size:28px;font-weight:700;letter-spacing:2px;">ICONIK</h1>
<p style="margin:8px 0 0;color:rgba(255,255,255,0.72);font-size:12px;letter-spacing:3px;text-transform:uppercase;">Style Intelligence System</p>
</td></tr>
<tr><td style="padding:32px 40px 0;text-align:center;"><div style="display:inline-block;background:#f0fdf4;border:2px solid #22c55e;border-radius:50px;padding:10px 24px;"><span style="color:#16a34a;font-size:15px;font-weight:600;">Order confirmed</span></div></td></tr>
<tr><td style="padding:28px 40px 0;">
<p style="margin:0 0 12px;color:#333;font-size:16px;line-height:1.7;">Hi ${firstName},</p>
<p style="margin:0 0 12px;color:#333;font-size:16px;line-height:1.7;">Thank you for purchasing your <strong>ICONIK Style Blueprint</strong>. Your payment is confirmed.</p>
<p style="margin:0;color:#555;font-size:15px;line-height:1.7;"><strong style="color:#b58e4d;">One thing stands between you and your Blueprint:</strong> completing the intake form so we can personalise your analysis.</p>
</td></tr>
<tr><td style="padding:20px 40px 0;">
...
<tr><td style="padding:32px 40px 40px;text-align:center;border-top:1px solid #eee6d8;">
<p style="margin:0 0 4px;color:#b58e4d;font-weight:700;font-size:15px;">ICONIK Style Intelligence</p>
<p style="margin:0;color:#999;font-size:13px;">help.iconikfashion@gmail.com</p>
<p style="margin:16px 0 0;color:#bbb;font-size:12px;">© 2026 ICONIK. All rights reserved.</p>
</td></tr>
</table></td></tr></table>
    ...
    console.log(`Stylist order confirmation sent to ${data.customer_email}. ID: ${info.messageId}`);
    return { success: true };
  } catch (error) {
    console.error('Error sending Stylist order confirmation email:', error);
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
  }
}
