...
function prescriptionPrompt(reportData: StylistBlueprintReportData, slot: keyof MutablePaths['prescription']) {
  ...
  const prompts: Record<keyof MutablePaths['prescription'], string> = {
    ...
    hairDirections: `Use the uploaded client headshot as the source image. Preserve the client's identity, face, skin tone, facial features, and natural proportions in every cell. Create one clean square 2x2 grid image with exactly four polished head-and-shoulders hairstyle options for the same client, in this order (top-left, top-right, bottom-left, bottom-right): ${hairStyles}. Each cell should show one distinct realistic, wearable hairstyle chosen for her ${reportData.classification.face_hair_accessories.face_shape} face shape, keeping her own hair texture and colour. Fill the square composition evenly with no blank bands. Matte ICONIK slate #94A6AD background. No text, no labels, no numbers, no before/after captions.`,
