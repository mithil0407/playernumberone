...
import type { ReportData, ReportSections } from '@/lib/globeReportGenerator';
import type { ResolvedImageUrls, FaceImageKind } from '@/lib/globeImageGenerator';
...
export default function AdminReportPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  const load = useCallback(async (options?: { fresh?: boolean; force?: boolean }) => {
    ...
    const res  = await fetch(`/api/globe-report/${reportId}${suffix}`, { cache: 'no-store' });
    ...
    if (data.report) {
      ...
      if (data.report.status === 'draft_ready') {
        await fetch(`/api/globe-report/${reportId}`, {
          method: 'PATCH',
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        const res = await fetch(`/api/globe-report/status/${reportId}`, { cache: 'no-store' });
        if (!res.ok) return;
  ...
  const saveEdit = async () => {
    ...
    const res  = await fetch(`/api/globe-report/${reportId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const copyLink = () => {
    if (!report) return;
    const url = `${window.location.origin}/globe/report/${report.share_token}`;
  ...
  const regenerateOutfit = useCallback(async (
    ...
    const res = await fetch(`/api/globe-report/${reportId}/regenerate-outfit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const regenerateFaceImage = useCallback(async (
    ...
    const res = await fetch(`/api/globe-report/${reportId}/regenerate-face-image`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleRejectAndRetry = async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/globe-report/generate/${report.submission_id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.reportId) router.push(`/globe/admin/report/${data.reportId}`);
    } catch {
  ...
  useEffect(() => {
    ...
    setTerminating(true);
    fetch(`/api/globe-report/${reportId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        status: 'error',
  ...
  const handleRetry = async () => {
    ...
    setError('');
    try {
      const res  = await fetch(`/api/globe-report/generate/${submissionId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageModel }),
      });
      ...
      if (data.reportId && data.reportId !== reportId) {
        router.push(`/globe/admin/report/${data.reportId}`);
      } else {
        await load();
      }
    } catch {
  ...
  const handleGenerateImages = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/globe-report/${reportId}/generate-images`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageModel }),
      });
  ...
  if (!report) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/globe/admin/dashboard" className="text-sm mt-4 inline-block" style={{ color: '#c9a96e' }}>
          ← Back to dashboard
        </Link>
      </div>
    );
      ...
      <div className="w-72 flex-shrink-0 flex flex-col border-l overflow-hidden" style={{ background: '#0f0f0f', borderColor: '#1e1e1e' }}>
        ...
        <div className="px-4 py-4 border-b" style={{ borderColor: '#1e1e1e' }}>
          ...
          <p className="text-xs mt-0.5 truncate" style={{ color: '#6b5f4a' }}>
            {report.globe_intake_submissions?.customer_email ?? reportId}
          </p>
          {isGenerating && (
              ...
              {report.share_token && (
                <div className="rounded-lg p-2.5 space-y-2" style={{ background: '#0d0d0d', border: '1px solid #2a2a2a' }}>
                  ...
                  <p className="text-[10px] break-all leading-relaxed" style={{ color: '#c8bfae' }}>
                    {`${window.location.origin}/globe/report/${report.share_token}`}
                  </p>
                  <div className="flex gap-1.5">
                    ...
                    <a
                      href={`/globe/report/${report.share_token}`}
                      target="_blank"
