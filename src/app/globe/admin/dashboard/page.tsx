...
export default function ManSubmissionsDashboard() {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/globe-admin/submissions?${params}`);
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {sub.latest_report?.status === 'generating' && sub.latest_report.id && (() => {
                        ...
                        return stuck ? (
                          ...
                          <Link
                            href={`/globe/admin/report/${sub.latest_report.id}`}
                      ...
                      {sub.latest_report?.status === 'sent' && (
                        <Link
                          href={`/globe/report/${sub.latest_report.share_token}`}
                          target="_blank"
                      ...
                      <Link
                        href={`/globe/admin/dashboard/${sub.id}`}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium transition-opacity hover:opacity-80"
                        style={{ background: '#1e1e1e', color: '#6b5f4a' }}
                      >
                        View
