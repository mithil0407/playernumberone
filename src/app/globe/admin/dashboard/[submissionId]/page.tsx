    ...
    <div className="rounded-2xl border p-5 flex items-center justify-between gap-4"
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {['draft_ready', 'in_review', 'approved', 'sent'].includes(report.status) && (
          <Link
            href={`/globe/admin/report/${report.id}`}
...
export default function SubmissionDetailPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  const [reports, setReports]       = useState<GlobeReport[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res  = await fetch(`/api/globe-admin/submissions/${submissionId}`);
  ...
  const handleGenerate = async () => {
    ...
    setGenError('');
    try {
      const res  = await fetch(`/api/globe-report/generate/${submissionId}`, { method: 'POST' });
      const data = await res.json();
