...
function GlobeThankyouInner() {
    const searchParams = useSearchParams();
    const paymentId = searchParams.get('payment_id') || '';
    const orderAmount = parseInt(searchParams.get('amount') || '199', 10);
    const rawEmail = searchParams.get('email') || '';
    const rawPhone = searchParams.get('phone') || '';

    const email = rawEmail || (typeof window !== 'undefined' ? localStorage.getItem('globe_customerEmail') : '') || '';
    const phone = rawPhone || (typeof window !== 'undefined' ? localStorage.getItem('globe_customerPhone') : '') || '';
    ...
    const handleSubscribe = useCallback(async () => {
        ...
        try {
            const response = await fetch('/api/globe-subscription', {
                method: 'POST',
            ...
            console.error('Globe subscription error:', err);
            setIsSubscribing(false);
...
export default function GlobeThankyouPage() {
    return (
