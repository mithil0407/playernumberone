...
export default function CheckoutPage() {
  ...
  // Product pricing. Keep the historical presentation, but use the current
  // shared pricing contract so the browser and payment API cannot drift.
  const originalPrice = 5999;
  const discountedPrice = INDIA_ROOT_BLUEPRINT_PRICE;
  const savings = originalPrice - discountedPrice;

  // Global route tracking owns PageView; checkout records only ViewContent.
  useEffect(() => {
    trackViewContent(
      'ICONIK Style Blueprint - Checkout',
      discountedPrice,
      [INDIA_BLUEPRINT_PRODUCT_ID],
      'INR',
      INDIA_ROOT_FUNNEL_CATEGORY,
  ...
  const wardrobeDetoxPrice = INDIA_BLUEPRINT_ADDON_PRICES.wardrobeDetox;
  const smartShoppersGuidePrice = INDIA_BLUEPRINT_ADDON_PRICES.smartShopper;
  const outfitPreviewPrice = INDIA_BLUEPRINT_ADDON_PRICES.outfitPreview;

  // Memoize total calculations to prevent unnecessary recalculations
  const totalAmount = useMemo(() =>
    calculateIndiaBlueprintTotal(discountedPrice, {
  ...
  const totalValue = useMemo(() =>
    ...
    [originalPrice, wardrobeDetoxAddon, wardrobeDetoxPrice, smartShoppersGuideAddon, smartShoppersGuidePrice, outfitPreviewAddon, outfitPreviewPrice]
  );
  ...
  const addonDetails = useMemo(() => ({
    wardrobedetox: {
      name: 'Wardrobe Detox',
      price: wardrobeDetoxPrice,
      id: INDIA_WARDROBE_DETOX_PRODUCT_ID,
    },
    smartshopper: {
      name: 'Smart Shopper\'s Guide',
      price: smartShoppersGuidePrice,
      id: INDIA_SMART_SHOPPER_PRODUCT_ID,
    },
    outfitpreview: {
      name: 'Outfit Preview on You',
      price: outfitPreviewPrice,
      id: INDIA_OUTFIT_PREVIEW_PRODUCT_ID,
    }
  }), [outfitPreviewPrice, smartShoppersGuidePrice, wardrobeDetoxPrice]);
  ...
  const handleAddonChange = useCallback((addonType: 'wardrobedetox' | 'smartshopper' | 'outfitpreview', checked: boolean) => {
    ...
    // Track addon changes
    if (checked) {
      trackAddToCart(addon.name, addon.price, addon.id, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
    } else {
      trackRemoveFromCart(addon.name, addon.price, addon.id, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
    }
  ...
  const processPayment = useCallback(async () => {
    ...
    const contentIds = buildIndiaBlueprintContentIds(selectedAddons);

    updateUserData(formData.email, formData.phone, INDIA_PHONE_COUNTRY_CODE);
    trackCTAClick('Pay Securely', 'Root Checkout', totalAmount, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
    ...
    try {
      ...
      const orderData = {
        ...
        amount: totalAmount,
        checkout_source: 'root_checkout',
        funnel_entry: INDIA_ROOT_FUNNEL_CATEGORY,
      ...
      const initializeRazorpay = () => {
        ...
        const options: RazorpayOptions = {
          ...
          handler: function (response: RazorpayResponse) {
            ...
            // The Razorpay payment ID is also used by the webhook's CAPI
            // Purchase, allowing Meta to deduplicate browser and server events.
            trackPurchase(
              totalAmount,
              INDIA_BLUEPRINT_CONTENT_NAME,
              contentIds,
            ...
            const successUrl = `/checkout/success?payment_id=${encodeURIComponent(paymentId)}`;
        ...
        const razorpay = new paymentWindow.Razorpay(options);
        razorpay.open();
      };
  ...
  }, [discountedPrice, formData, outfitPreviewAddon, outfitPreviewPrice, razorpayLoaded, smartShoppersGuideAddon, smartShoppersGuidePrice, totalAmount, wardrobeDetoxAddon, wardrobeDetoxPrice]);
            ...
            <div className="bg-luxury-warm-white border-2 border-luxury-charcoal text-luxury-charcoal rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-2">
                  ...
                  <li className="flex items-start gap-2 md:gap-3">
                    <CheckCircle className="w-4 h-4 md:w-5 md:h-5 text-luxury-accent flex-shrink-0 mt-0.5" />
                    <span className="luxury-body text-luxury-charcoal/80 text-sm md:text-base">30-min Private Consultation Call</span>
                  </li>
                  <li className="flex items-start gap-2 md:gap-3">
                    <CheckCircle className="w-4 h-4 md:w-5 md:h-5 text-luxury-accent flex-shrink-0 mt-0.5" />
            ...
            <div className="bg-luxury-cream/40 backdrop-blur-xl rounded-3xl p-5 md:p-6 border border-luxury-cream">
              ...
              <button
                ...
                onClick={async (e) => {
                  ...
                  if (!hasAddons && !popupDismissed) {
                    setShowAddonPopup(true);
                    trackCTAClick('Add-on Popup Shown', 'Checkout Main Button', totalAmount, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
                  } else {
                    await processPayment();
                  }
                }}
          ...
          >
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  onClick={() => {
                    ...
                    trackCTAClick('Add-on Popup Dismissed', 'Popup Close Button', undefined, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
                  }}
            ...
            <div className="sticky bottom-0 bg-luxury-warm-white/95 backdrop-blur-xl border-t border-luxury-cream p-3 md:p-4 space-y-2">
              <button
                onClick={async () => {
                  setShowAddonPopup(false);
                  trackCTAClick('Proceed with Add-ons', 'Popup Continue Button', totalAmount, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
                  await processPayment();
                }}
                disabled={isProcessing}
              ...
              <button
                onClick={() => {
                  trackCTAClick('Continue without Add-ons', 'Popup Skip Button', totalAmount, 'INR', INDIA_ROOT_FUNNEL_CATEGORY);
