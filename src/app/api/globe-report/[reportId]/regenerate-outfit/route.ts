...
export async function POST(
  ...
  const { data: report, error } = await supabaseGlobeServer
    .from('globe_reports')
  ...
  const imagePaths = report.image_urls as GlobeReportImagePaths | null;
  ...
  if (hairstylePath) {
    const { data: signed } = await supabaseGlobeServer.storage
      .from('globe-report-images')
  ...
  const newImagePaths = await mergeGlobeReportImagePathsForReport(
    reportId,
  ...
  const resolved = await resolveGlobeReportImageUrls(newImagePaths);
