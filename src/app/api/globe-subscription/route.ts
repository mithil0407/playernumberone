// Globe Subscription API — ICONIK Style Feed OTO (USD)
...
// Globe Style Feed plan IDs (Razorpay — USD)
const GLOBE_STYLE_FEED_PLANS = {
    monthly: 'plan_SL2xA4EzLOvyR1',  // USD $19/month
    annual: 'plan_SL2y152SKsukWW',    // USD $168/year
};

const GLOBE_PLAN_AMOUNTS = {
...
export async function POST(request: NextRequest) {
    try {
        ...
        const planId = GLOBE_STYLE_FEED_PLANS[plan_type as keyof typeof GLOBE_STYLE_FEED_PLANS];
        const amount = GLOBE_PLAN_AMOUNTS[plan_type as keyof typeof GLOBE_PLAN_AMOUNTS];
        ...
        const subscriptionRequest = {
            ...
            notes: {
                ...
                plan_type,
                product: 'ICONIK Style Feed — Globe',
                region: 'Globe',
        ...
        try {
            ...
            if (!existingSub) {
                await saveGlobeSubscription({
                    ...
                    status: 'pending',
                    source: 'globe_oto',
                    notes: `Globe OTO — ${plan_type} plan created at ${new Date().toISOString()}`,
                    ...subscriptionAttribution,
                });
            }
        } catch (dbError) {
            console.error('Failed to save Globe subscription to Supabase:', dbError);
        }
        ...
        console.error('Globe subscription API error:', error);
        return NextResponse.json(
            {
                success: false,
