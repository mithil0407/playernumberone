// Global Quiz Reminder Cron
// Called by a cron job (e.g. cron-job.org) via GET request.
// Finds paid globe_orders (USD/Global) with no intake submission (30–90 min window) and sends a reminder.
...
async function handleReminder(request: NextRequest) {
    ...
    try {
        ...
        const lookbackHours = Number(process.env.GLOBAL_QUIZ_REMINDER_LOOKBACK_HOURS || 24);
        const maxOrdersPerRun = Number(process.env.GLOBAL_QUIZ_REMINDER_BATCH_SIZE || 50);
        ...
        // ── Find paid globe orders (USD/Global) old enough for a reminder ───
        ...
        if (ordersError) {
            console.error('Global quiz reminder — orders query error:', ordersError);
        ...
        console.log(`Global quiz reminder: found ${orders.length} eligible order(s)`);
        ...
        if (stylistOrdersRes.error || stylistIntakesRes.error) {
            console.error('Global quiz reminder — stylist intake query error:', stylistOrdersRes.error || stylistIntakesRes.error);
        ...
        for (const order of orders) {
            ...
            if (result.success) {
                reminded++;
                console.log(`Global quiz reminder sent to ${email}`);
            } else {
                console.error(`Global quiz reminder failed for ${email}:`, result.error);
            }
        }
        ...
        console.error('Global quiz reminder cron error:', error);
