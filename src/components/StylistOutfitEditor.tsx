...
import { ArrowRight, Check, Copy, Crop, ImagePlus, Loader2, Plus, Trash2, WandSparkles } from 'lucide-react';
...
export default function StylistOutfitEditor({ page, onChange, onSave, onUpload, onParse, getPrompt, getAlternatives, chooseAlternative, onApprove, imageUrl, busy, saving, hasUnsavedEdits, saveError, getCropAspect }: {
  saveError?: string;
  ...
  // Any edit to the outfit makes the prompt on screen wrong, and the prompt is
  // what the stylist pastes into the image tool — so drop it and let step 02
  // fetch the one that matches what is actually saved.
  const update = (next: BlueprintPage) => { setPrompt(null); onChange(next); };
  const changePiece = (index: number, patch: Partial<OutfitPiece>) => update(setOutfitPieces(page, pieces.map((piece, i) => i === index ? { ...piece, ...patch } : piece)));
  ...
  const applyPastedOutfit = async () => {
    const text = outfitText.trim();
    if (!text) { setError('Paste the outfit first.'); return; }
    setWorking(true); setError(''); setMessage('');
    try {
      const nextPrompt = await onParse(text);
      setOutfitText('');
      setPrompt(nextPrompt);
      setMessage('Outfit updated below. Check the pieces, then continue to the image.');
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Could not read that outfit.');
    } finally { setWorking(false); }
  };
  const loadPrompt = useCallback(async () => {
    setPromptLoading(true);
    try { setPrompt(await getPrompt()); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not load the image prompt.'); }
    finally { setPromptLoading(false); }
  }, [getPrompt]);
  useEffect(() => {
    if (step !== 'image' || prompt !== null || promptLoading || busy || saving) return;
    void loadPrompt();
  }, [step, prompt, promptLoading, busy, saving, loadPrompt]);
  ...
  const copyPrompt = async () => {
    setWorking(true); setError('');
    try {
      await navigator.clipboard.writeText(prompt ?? await getPrompt());
      setMessage('Image prompt copied. Paste it into your image tool, then upload the result below.');
    }
  ...
  return <div className="luxury-body space-y-5">
    <div className="grid grid-cols-2 rounded-xl p-1 bg-[#EDE5D2]" aria-label="Outfit workflow">
      ...
      <button onClick={() => void continueToImage()} disabled={disabled || saving} className={`rounded-lg px-3 py-3 text-sm disabled:opacity-50 ${step === 'image' ? 'bg-[#2C2622] text-[#F4EFE5]' : ''}`}>02 · Add the image</button>
    </div>
    <p className="text-xs leading-5 text-[#746D65]">{step === 'outfit' ? 'Paste the outfit you want and it fills in the pieces below. Your report preview updates as you go.' : 'Copy the image prompt, make the image, then upload it here.'}</p>
    ...
    {step === 'outfit' ? <>
      <div className="rounded-2xl border border-[#2C2622]/10 bg-[#EDE5D2]/60 p-4">
        <p className="text-sm font-medium">Write the outfit</p>
        <p className="text-xs leading-5 text-[#746D65] mt-1">Type or paste it however you like — a list or a sentence. Every piece, colour and note below is filled in for you, and so is the image prompt.</p>
        <textarea
          rows={5}
          className={fieldClass}
          value={outfitText}
          disabled={disabled || saving}
          placeholder={'Ivory silk shirt, charcoal wide-leg trousers,\ntan leather loafers, gold hoops, cognac structured tote'}
          onChange={event => setOutfitText(event.target.value)}
        />
        <button
          type="button"
          onClick={() => void applyPastedOutfit()}
          disabled={disabled || saving || !outfitText.trim()}
          className="w-full mt-3 flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm bg-[#2C2622] text-[#F4EFE5] disabled:opacity-50"
        >
          {working ? <Loader2 size={16} className="animate-spin" /> : <WandSparkles size={16} />}
      ...
      <div className="rounded-2xl border border-[#2C2622]/10 bg-[#EDE5D2]/60 p-4">
        ...
        {alternatives && <div className="space-y-2 mt-4">
          ...
          {alternatives.map((option, index) => <button key={option.id} disabled={disabled} onClick={async () => {
            setWorking(true); setError('');
            try { await chooseAlternative(option.id); setAlternatives(null); setPrompt(null); setMessage('New outfit saved. Review the pieces, then add its image.'); }
      ...
      <fieldset disabled={disabled} className="space-y-5">
        ...
        <button type="button" disabled={pieces.length >= 8} onClick={() => update(setOutfitPieces(page, [...pieces, { slot: 'Accessory', piece: '', colour_name: '', colour_hex: '#2C2622', palette_role: 'accent', structural_notes: '' }]))} className="inline-flex items-center gap-2 text-sm underline disabled:opacity-40"><Plus size={15} /> Add a piece</button>
        {OUTFIT_COPY_FIELDS.map(field => <label key={field.key} className="block text-xs font-medium">{field.label}<textarea rows={3} className={fieldClass} value={outfitCopy(page, field.key)} onChange={e => update(setOutfitCopy(page, field.key, e.target.value))} /></label>)}
      </fieldset>
      ...
      <div className="rounded-2xl border border-[#2C2622]/10 bg-[#EDE5D2]/60 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button type="button" disabled={disabled || saving || (!prompt && promptLoading)} onClick={() => void copyPrompt()} className="inline-flex items-center gap-2 rounded-lg border border-[#2C2622]/15 bg-[#FAF7F0] px-3 py-2 text-xs disabled:opacity-50"><Copy size={13} /> Copy</button>
        </div>
        <p className="text-xs leading-5 text-[#746D65] mt-1">Written from the outfit you just saved. It updates itself whenever the pieces change.</p>
        {promptLoading
          ? <p className="mt-3 inline-flex items-center gap-2 text-xs text-[#746D65]"><Loader2 size={13} className="animate-spin" /> Writing the prompt…</p>
          : prompt
            ? <textarea readOnly aria-label="Image prompt" rows={9} value={prompt} className={`${fieldClass} font-mono text-[11px] leading-5`} onFocus={event => event.currentTarget.select()} />
            : <button type="button" onClick={() => void loadPrompt()} className="mt-3 text-xs underline">Show the image prompt</button>}
      </div>
