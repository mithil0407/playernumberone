...
const NAV = [
  { href: '/man/admin/dashboard', label: 'Blueprints', icon: LayoutDashboard },
  { href: '/man/admin/edit', label: 'ICONIK Edit', icon: Mail },
];
...
export default function ManAdminShell({ children }: { children: React.ReactNode }) {
  ...
  if (pathname.startsWith('/man/admin/report/')) {
    ...
    <div className="min-h-screen flex" style={{ background: S.bg, fontFamily: 'var(--font-inter, system-ui)' }}>
      ...
      >
        <div className="px-5 py-5" style={{ borderBottom: `1px solid ${S.border}` }}>
          ...
          <div className="iconik-micro mt-1.5" style={{ color: S.muted }}>Man · Admin</div>
        </div>
