...
// GlobeReport.tsx
// Renders the full ICONIK Globe women's Blueprint report.

import { useState, useMemo, memo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Pencil, X, Loader2, RefreshCw, AlertCircle } from 'lucide-react';
import type { ReportData, ClassificationResult } from '@/lib/globeReportGenerator';
import type { ResolvedImageUrls } from '@/lib/globeImageGenerator';
import { SPRING } from '@/lib/reportAnimations';
import { useIntersectionObserver } from '@/hooks/useIntersectionObserver';
import type { FaceImageKind } from '@/lib/globeImageGenerator';

// ─────────────────────────────────────────────────────────────
// Design tokens (matches /globe/page.tsx embedded report)
// ─────────────────────────────────────────────────────────────
...
const KNOWN_CONTEXTS = new Set(['WORK', 'SMART CASUAL', 'EVENING', 'WEEKEND', 'FORMAL OCCASION', 'TRAVEL RESORT']);
...
function parseOutfitCategories(text: string): OutfitCategory[] {
  ...
  for (const block of outfitBlocks) {
    ...
    const outfit: ParsedOutfit = {
      ...
      singlePiece: getField(block, 'Single Piece'),
      layer:       getField(block, 'Layer(?:\\/Outerwear)?(?:\\/Layer)?'),
      footwear:    getField(block, 'Footwear'),
      bag:         getField(block, 'Bag'),
      jewellery:   getField(block, 'Jewellery|Jewelry'),
  ...
  return (
    <div className="bg-white border-b" style={{ borderColor: BORDER }}>
      ...
      <div className="p-6 md:p-10">
        ...
        {/* Sunglasses / eyeframe images — two headshots side by side */}
        {(hasEyewearImages || (adminMode && !!onRegenerateFaceImage)) && (
          <div className="mb-8">
            <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em] mb-4">Your Sunglasses & Eyeframe Options</p>
            <div className="grid grid-cols-2 gap-4">
              {[1, 2].map(optionIndex =>
        ...
        {/* Frame, neckline, and earring guidance */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t" style={{ borderColor: BORDER }}>
          <div>
            <DataLabel>Sunglasses & Frames</DataLabel>
            <span className="text-xs text-black font-light">{face.eyewear_shapes.join(' · ')}</span>
          </div>
          <div>
            <DataLabel>Necklines & Earrings</DataLabel>
            <span className="text-xs text-black font-light">
              {[face.neckline_guidance, face.earring_guidance].filter(Boolean).join(' · ') || 'Guided by the section notes below'}
            </span>
          </div>
        </div>
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2.5">
                    {[
                      { label: 'Single',      value: outfit.singlePiece },
                      ...
                      { label: 'Jewellery',   value: outfit.jewellery },
...
export default memo(GlobeReport);
